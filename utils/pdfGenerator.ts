...
async function getLogos(): Promise<{ vulcan: string | null; app: string | null }> {
...
export async function generateAccessDocument(
    ...
    if (process.requestType === 'PersonnelAccess') {
        await generatePersonnelBadgePDF(process, language);
    } else if (process.requestType === 'EquipmentAccess') {
        await generateEquipmentDiscPDF(process, language);
    } else if (process.requestType === 'DeliveryAccess') {
        await generateDeliveryAccessPDF(process, language);
    } else {
        // Recruitment (temporal access / contractor)
        await generateTemporalAccessPDF(process, language);
    }
}
