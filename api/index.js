...
// ─── PostgreSQL Connection (Singleton for warm starts) ──────────────────────────
...
function matchRoute(method, pattern, reqMethod, reqPath) {
    ...
    if (!pattern.includes(':')) return reqPath === pattern ? {} : null;
...
module.exports = async (req, res) => {
    ...
    try {
        ...
        if ((params = matchRoute('DELETE', '/api/sites/:id', req.method, pathname))) {
            await db.query('DELETE FROM sites WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('DELETE', '/api/employees/:id', req.method, pathname))) {
            await db.query('DELETE FROM employees WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('PUT', '/api/users/:id/password', req.method, pathname))) {
            await db.query('UPDATE users SET password = $1 WHERE id = $2', [body.password, params.id]);
            return sendJson(res, 200, { success: true });
        }

        if ((params = matchRoute('DELETE', '/api/users/:id', req.method, pathname))) {
            await db.query('DELETE FROM users WHERE id = $1', [parseInt(params.id)]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('DELETE', '/api/sessions/:id', req.method, pathname))) {
            await db.query('DELETE FROM training_sessions WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('POST', '/api/waitlist/:id/promote', req.method, pathname))) {
            ...
            await db.query('DELETE FROM waiting_list WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('GET', '/api/rac-definitions', req.method, pathname))) {
            const { rows } = await db.query('SELECT * FROM rac_definitions ORDER BY code');
            return sendJson(res, 200, rows);
        }
        ...
        if ((params = matchRoute('DELETE', '/api/rac-definitions/:id', req.method, pathname))) {
            await db.query('DELETE FROM rac_definitions WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('DELETE', '/api/rooms/:id', req.method, pathname))) {
            await db.query('DELETE FROM rooms WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('GET', '/api/trainers', req.method, pathname))) {
            const { rows } = await db.query('SELECT * FROM trainers ORDER BY name');
            return sendJson(res, 200, rows);
        }
        ...
        if ((params = matchRoute('DELETE', '/api/trainers/:id', req.method, pathname))) {
            await db.query('DELETE FROM trainers WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
        ...
        if ((params = matchRoute('GET', '/api/connectors', req.method, pathname))) {
            const { rows } = await db.query('SELECT * FROM data_connectors ORDER BY name');
            return sendJson(res, 200, rows);
        }
        ...
        if ((params = matchRoute('GET', '/api/unsafe-conditions', req.method, pathname))) {
            const { rows } = await db.query('SELECT * FROM unsafe_conditions ORDER BY created_at DESC');
            return sendJson(res, 200, rows);
        }
        ...
        if ((params = matchRoute('GET', '/api/recruitment-processes', req.method, pathname))) {
            const { rows } = await db.query('SELECT * FROM recruitment_processes ORDER BY requested_at DESC');
            return sendJson(res, 200, rows);
        }
        ...
        if ((params = matchRoute('DELETE', '/api/recruitment-processes/all', req.method, pathname))) {
            await db.query('DELETE FROM recruitment_processes');
            return sendJson(res, 200, { success: true });
        }

        if ((params = matchRoute('DELETE', '/api/recruitment-processes/:id', req.method, pathname))) {
            await db.query('DELETE FROM recruitment_processes WHERE id = $1', [params.id]);
            return sendJson(res, 200, { success: true });
        }
