...
export function SecurityPortalPage() {
    ...
    function issueBadge(p: RecruitmentProcess) {
        if (p.requestType === 'DeliveryAccess') {
            ...
            generateAccessDocument(updated, language).catch(err => console.error('PDF generation error:', err));
            resetAccessForm();
            return;
        }
    ...
    return (
        <div className="p-6 max-w-[1600px] mx-auto space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <button onClick={() => { setProcesses(loadProcesses()); toast(language === 'pt' ? 'Atualizado.' : 'Refreshed.'); }}
            ...
            <div className="p-6 space-y-6 max-h-[580px] overflow-y-auto">
                ...
                {p.accessDocumentRef && (
                    <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-2xl">
                        <button onClick={() => generateAccessDocument(p, language).catch(err => console.error('PDF error:', err))}
