...
export function HRPortalPage() {
    ...
    function receiveRequest(p: RecruitmentProcess) {
        ...
        toast(`${p.candidateName} received into HR queue.`);
    }
    ...
    function handleDocUpload(process: RecruitmentProcess, docType: 'ID' | 'Passport' | 'Work Permit', e: React.ChangeEvent<HTMLInputElement>) {
        ...
        setTimeout(() => {
            ...
            toast(`${docType} uploaded and verified.`);
        }, 1200);
        e.target.value = '';
    }
    ...
    function approveToSecurity(p: RecruitmentProcess) {
        const nextStatus = p.workerType === 'Contractor'
            ? RecruitmentStatus.PARALLEL_CLEARANCE_PENDING
            : RecruitmentStatus.SECURITY_PENDING;
        update({ ...p, status: nextStatus });
        toast(`${p.candidateName} forwarded to ${p.workerType === 'Contractor' ? 'Security + Clinic (Parallel)' : 'Security'}.`);
    }

    // Reject / Return to AM
    function returnToAM(p: RecruitmentProcess) {
        update({ ...p, status: RecruitmentStatus.AM_REQUESTED });
        toast(`${p.candidateName} returned to Area Manager for revision.`);
    }

    const stats = [
        { label: 'Pending AM Request', value: processes.filter(p => p.status === RecruitmentStatus.AM_REQUESTED && (p.requestType === 'Recruitment' || !p.requestType)).length, color: 'text-slate-700', bg: 'bg-slate-50', border: 'border-slate-200' },
        { label: 'In HR Review', value: processes.filter(p => p.status === RecruitmentStatus.HR_PENDING && (p.requestType === 'Recruitment' || !p.requestType)).length, color: 'text-amber-700', bg: 'bg-amber-50', border: 'border-amber-200' },
        { label: 'Approved This Month', value: processes.filter(p => p.status !== RecruitmentStatus.AM_REQUESTED && p.status !== RecruitmentStatus.HR_PENDING && (p.requestType === 'Recruitment' || !p.requestType)).length, color: 'text-emerald-700', bg: 'bg-emerald-50', border: 'border-emerald-200' },
        { label: 'Total Pipeline', value: processes.filter(p => p.requestType === 'Recruitment' || !p.requestType).length, color: 'text-indigo-700', bg: 'bg-indigo-50', border: 'border-indigo-200' },
    ];

    const tabs = [
        { id: 'queue', label: 'Active Queue', icon: Inbox, count: queue.length },
        { id: 'history', label: 'Processed', icon: History, count: history.length },
        { id: 'documents', label: 'All Documents', icon: FileText, count: all.reduce((a, p) => a + (p.documents?.length || 0) + (p.amDocuments?.length || 0), 0) },
    ] as const;
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 ml-14">
                        Review AM requests · Verify documents · Forward to Security clearance
                    </p>
                </div>
            ...
            <div className="flex gap-1 p-1 bg-slate-100 dark:bg-slate-800/80 rounded-2xl w-fit">
                {tabs.map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        ...
                        <span className={`text-[9px] px-1.5 py-0.5 rounded-full font-black ${activeTab === tab.id ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-200 text-slate-500'}`}>{tab.count}</span>
                    </button>
                ))}
            </div>
                                ...
                                <div className="flex items-start justify-between gap-2">
                                    <div className="min-w-0">
                                        ...
                                        <div className="flex items-center gap-1.5 mt-2">
                                            ...
                                            <span className={`text-[9px] font-black px-2 py-0.5 rounded-full ${p.workerType === 'Contractor' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>{p.workerType}</span>
                                        </div>
                                    </div>
                ...
                <div className="lg:col-span-8">
                    {selected ? (
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-[2rem] border border-slate-200 dark:border-slate-700 p-16 text-center text-slate-400 shadow-sm h-full flex flex-col items-center justify-center">
                            ...
                            <p className="text-sm font-bold">Select a candidate from the list</p>
                            <p className="text-xs mt-1">to review documents and take action</p>
                        </div>
                    )}
                </div>
            </div>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2 mt-1.5">
                                ...
                                <span className={`text-[9px] font-black px-2.5 py-1 rounded-full ${p.workerType === 'Contractor' ? 'bg-amber-100 text-amber-700' : 'bg-blue-100 text-blue-700'}`}>
                                    {p.workerType === 'Contractor' ? '🤝 Contractor' : '🏢 Prime'}
                                </span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                        {p.status === RecruitmentStatus.AM_REQUESTED && (
                            <button onClick={() => onReceive(p)}
                                ...
                                <Inbox size={13} /> Receive Into Queue
                            </button>
                        )}
                        {p.status === RecruitmentStatus.HR_PENDING && (
                            <>
                                <button onClick={() => onApprove(p)}
                                    ...
                                    <CheckCircle2 size={13} /> Approve → Security
                                </button>
            ...
            <div className="p-6 space-y-6 max-h-[580px] overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        { icon: Building2, label: p.workerType === 'Contractor' ? 'Contractor Co.' : 'Company', value: p.contractorCompany || p.primeCompany },
                ...
                {/* Required RACs */}
                <div>
                    <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Required RAC Training</div>
                    <div className="flex flex-wrap gap-2">
                        ...
                        {(p.requiredRacs || []).length === 0 && <span className="text-xs text-slate-400">No RACs specified</span>}
                    </div>
                </div>
                ...
                {(p.amDocuments || []).length > 0 && (
                    <div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                            <FileScan size={11} /> Documents from Area Manager
                        </div>
                        <div className="space-y-2">
                ...
                {isActionable && p.status === RecruitmentStatus.HR_PENDING && (
                    <div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                            <Upload size={11} /> HR Document Uploads
                        </div>
                        <div className="space-y-2">
                                ...
                                return (
                                    <div key={docType} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 rounded-xl">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div>
                                                <div className="text-xs font-black text-slate-700 dark:text-slate-300">{docType}</div>
                                                {existing ? (
                                                    <div className="text-[9px] text-emerald-600 font-bold">{existing.name} · {existing.fileSize}</div>
                                                ) : (
                                                    <div className="text-[9px] text-slate-400">PDF, JPG, PNG · max 10 MB</div>
                                                )}
                                            </div>
                                        </div>
                                        {existing ? (
                                            <span className="text-[9px] bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-black flex items-center gap-1"><Check size={9}/> Uploaded</span>
                                        ) : (
                ...
                {(p.documents || []).filter(d => d.uploadedBy === 'HR').length > 0 && p.status !== RecruitmentStatus.HR_PENDING && (
                    <div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">HR Verified Documents</div>
                        <div className="space-y-2">
                            {(p.documents || []).filter(d => d.uploadedBy === 'HR').map((doc, i) => (
                                <div key={i} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-100 dark:border-slate-700">
                                    ...
                                    <span className="text-[9px] bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded-full font-black">Verified by HR</span>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {p.workerType === 'Contractor' && (
                    <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl">
                        <div className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-2 flex items-center gap-1.5">
                            <AlertTriangle size={11} /> Contractor — Parallel Clearance Workflow
                        </div>
                        <p className="text-xs text-amber-800 dark:text-amber-300 font-medium leading-relaxed">
                            Once approved, this request will be forwarded simultaneously to <strong>Security</strong> (access badge) and <strong>Clinic</strong> (fitness verification). Both departments must clear before induction begins.
                        </p>
                        {p.contractorCompany && (
                            <div className="mt-2 text-xs font-black text-amber-700">Contractor Company: {p.contractorCompany}</div>
                        )}
                    </div>
                )}
            </div>
