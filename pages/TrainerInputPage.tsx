...
const TrainerInputPage: React.FC<TrainerInputPageProps> = ({ 
  ...
  const getRacRequirements = (sessionId: string) => {
      ...
      return { needsDl: def ? !!def.requiresDriverLicense : false, needsPractical: def ? !!def.requiresPractical : true };
  };
  ...
  const processedBookings = useMemo(() => {
      ...
      if (statusFilter !== 'All') data = data.filter(b => statusFilter === 'Passed' ? b.status === BookingStatus.PASSED : b.status === BookingStatus.FAILED);
      data.sort((a, b) => {
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden flex flex-col min-h-[600px] print:shadow-none print:border-none print:min-h-0">
          ...
          {selectedSessionId && sessionBookings.length > 0 ? (
            <><div className="hidden print:block p-8 border-b-2 border-black mb-4"><div className="flex justify-between items-start"><div className="flex items-center gap-4"><div className="h-16 w-16 bg-slate-900 text-white rounded flex items-center justify-center font-black text-xs text-center p-1 leading-tight">{language === 'pt' ? 'RACS' : 'CARS'} SAFETY</div><div><h1 className="text-2xl font-black uppercase tracking-tight text-black">Training Result Register</h1><p className="text-sm font-medium text-gray-600 uppercase tracking-wider">Critical Activity Requirements (RAC)</p></div></div><div className="text-right"><div className="text-xl font-bold text-black">{currentSession?.racType}</div><div className="text-sm font-mono">{currentSession?.id}</div></div></div><div className="mt-8 grid grid-cols-4 gap-4 text-sm border-t border-gray-300 pt-4"><div><span className="block text-[10px] uppercase font-bold text-gray-500">Date</span><span className="font-bold text-black">{currentSession?.date}</span></div><div><span className="block text-[10px] uppercase font-bold text-gray-500">Time</span><span className="font-bold text-black">{currentSession?.startTime}</span></div><div><span className="block text-[10px] uppercase font-bold text-gray-500">Location</span><span className="font-bold text-black">{currentSession?.location}</span></div><div><span className="block text-[10px] uppercase font-bold text-gray-500">Instructor</span><span className="font-bold text-black">{currentSession?.instructor}</span></div></div></div>
                ...
                <div className="flex-1 overflow-x-auto"><table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700 border-collapse"><thead className="bg-slate-50 dark:bg-slate-800 print:bg-gray-100"><tr><th onClick={() => handleHeaderClick('Name')} className="px-4 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-700 dark:hover:text-slate-200 group print:text-black print:border-b print:border-black"><div className="flex items-center gap-1">Student <ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-100 ${sortBy === 'Name' ? 'opacity-100' : ''} no-print`}/></div></th><th onClick={() => handleHeaderClick('ID')} className="px-4 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-700 group hidden md:table-cell print:table-cell print:text-black print:border-b print:border-black"><div className="flex items-center gap-1">ID <ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-100 ${sortBy === 'ID' ? 'opacity-100' : ''} no-print`}/></div></th><th onClick={() => handleHeaderClick('Company')} className="px-4 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider cursor-pointer hover:text-slate-700 group hidden lg:table-cell print:table-cell print:text-black print:border-b print:border-black"><div className="flex items-center gap-1">Company <ArrowUpDown size={12} className={`opacity-0 group-hover:opacity-100 ${sortBy === 'Company' ? 'opacity-100' : ''} no-print`}/></div></th><th className="px-4 py-3 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-16 print:text-black print:border-b print:border-black">Present</th>{reqs.needsDl && <th className="px-4 py-3 text-center text-xs font-bold text-red-500 uppercase tracking-wider w-16 bg-red-50 dark:bg-red-900/10 print:bg-transparent print:text-black print:border-b print:border-black">DL Ver</th>}<th className="px-4 py-3 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-24 print:text-black print:border-b print:border-black">Theory</th>{reqs.needsPractical && <th className="px-4 py-3 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-24 print:text-black print:border-b print:border-black">Practical</th>}<th className="px-4 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-32 print:text-black print:border-b print:border-black">Status</th><th className="px-4 py-3 text-left text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider w-48 no-print">Remarks</th><th className="hidden print:table-cell px-4 py-3 text-center text-xs font-bold text-black uppercase tracking-wider w-32 border-b border-black">Signature</th></tr></thead><tbody className="divide-y divide-slate-100 dark:divide-slate-700 bg-white dark:bg-slate-800 print:divide-gray-300">{processedBookings.map((booking) => {const isDisqualified = reqs.needsDl && !booking.driverLicenseVerified; const isPassed = booking.status === BookingStatus.PASSED; return (<tr key={booking.id} className={`transition-colors ${isPassed ? 'bg-green-50/30 dark:bg-green-900/5 hover:bg-green-50/50 print:bg-transparent' : 'hover:bg-slate-50 dark:hover:bg-slate-700/50 print:bg-transparent'} print:break-inside-avoid`}><td className="px-4 py-3 print:py-2 print:border-b print:border-gray-200"><div className="font-bold text-slate-800 dark:text-white text-sm print:text-black">{booking.employee.name}</div></td><td className="px-4 py-3 hidden md:table-cell text-sm font-mono text-slate-600 dark:text-slate-300 print:table-cell print:text-black print:py-2 print:border-b print:border-gray-200">{booking.employee.recordId}</td><td className="px-4 py-3 hidden lg:table-cell text-xs text-slate-500 print:table-cell print:text-black print:py-2 print:border-b print:border-gray-200">{booking.employee.company}</td><td className="px-4 py-3 text-center print:py-2 print:border-b print:border-gray-200"><input type="checkbox" className="w-5 h-5 rounded text-blue-600 focus:ring-blue-500 border-gray-300 cursor-pointer" checked={booking.attendance || false} onChange={(e) => handleInputChange(booking.id, 'attendance', e.target.checked)}/></td>{reqs.needsDl && (<td className="px-4 py-3 text-center bg-red-50/30 dark:bg-red-900/10 print:bg-transparent print:py-2 print:border-b print:border-gray-200"><input type="checkbox" className="w-5 h-5 rounded text-red-600 focus:ring-red-500 border-gray-300 cursor-pointer" checked={booking.driverLicenseVerified || false} onChange={(e) => handleInputChange(booking.id, 'driverLicenseVerified', e.target.checked)}/></td>)}<td className="px-4 py-3 print:py-2 print:border-b print:border-gray-200"><input type="number" min="0" max="100" className={`w-full text-center p-1.5 text-sm font-bold border rounded-lg outline-none focus:ring-2 transition-all ${isDisqualified ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : (booking.theoryScore || 0) >= 70 ? 'border-green-300 bg-green-50 text-green-700' : 'border-red-300 bg-red-50 text-red-700'}`} value={booking.theoryScore} disabled={isDisqualified} onChange={(e) => handleInputChange(booking.id, 'theoryScore', parseInt(e.target.value) || 0)}/></td>{reqs.needsPractical && (<td className="px-4 py-3 relative print:py-2 print:border-b print:border-gray-200">{isDisqualified || (booking.theoryScore || 0) < 70 ? (<div className="absolute inset-0 flex items-center justify-center print:hidden"><Lock size={14} className="text-slate-300" /></div>) : (<input type="number" min="0" max="100" className={`w-full text-center p-1.5 text-sm font-bold border rounded-lg outline-none focus:ring-2 transition-all ${(booking.practicalScore || 0) >= 70 ? 'border-green-300 bg-green-50 text-green-700' : 'border-red-300 bg-red-50 text-red-700'}`} value={booking.practicalScore} onChange={(e) => handleInputChange(booking.id, 'practicalScore', parseInt(e.target.value) || 0)}/>)}</td>)}<td className="px-4 py-3 print:py-2 print:border-b print:border-gray-200"><span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-black uppercase tracking-wider border shadow-sm ${isPassed ? 'bg-green-100 border-green-200 text-green-700 dark:text-green-400' : 'bg-red-100 border-red-200 text-red-700 dark:text-red-400'}`}>{booking.status}</span></td><td className="px-4 py-3 no-print"><div className="relative"><input type="text" placeholder="Add remarks..." className="w-full bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-blue-500 outline-none text-xs py-1 text-slate-600 dark:text-slate-300" value={booking.comments || ''} onChange={(e) => handleInputChange(booking.id, 'comments', e.target.value)}/><MessageSquare size={10} className="absolute right-0 top-1.5 text-slate-300 pointer-events-none" /></div></td><th className="hidden print:table-cell px-4 py-3 border-b border-black"></th></tr>);})}</tbody></table></div>
