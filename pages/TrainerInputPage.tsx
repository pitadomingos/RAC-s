...
const TrainerInputPage: React.FC<TrainerInputPageProps> = ({ 
  ...
  // Helper to determine requirements based on selected session's RAC
  const getRacRequirements = (sessionId: string) => {
      const session = sessions.find(s => s.id === sessionId);
      if (!session) return { needsDl: false, needsPractical: true }; // Default safely
  ...
  const calculateStatus = (booking: Booking): BookingStatus => {
    const { needsDl, needsPractical } = getRacRequirements(booking.sessionId);
  ...
  const reqs = getRacRequirements(selectedSessionId);
         ...
         <div className="relative z-10">
            <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                <div>
                    ...
                    <p className="text-slate-400 mt-2 text-sm flex items-center gap-2"><UserCheck size={16} />{t.trainer.loggedInAs} <span className="text-white font-bold">{currentUserName || 'Admin'}</span></p>
                </div>
                {selectedSessionId && (
                    <div className="flex gap-4">
                        <div className="bg-white/10 p-3 rounded-xl border border-white/10 backdrop-blur-sm text-center min-w-[100px]"><div className="text-2xl font-bold">{sessionBookings.length}</div><div className="text-[10px] uppercase text-slate-400 font-bold">Pending</div></div>
                        <div className="bg-green-500/20 p-3 rounded-xl border border-green-500/30 backdrop-blur-sm text-center min-w-[100px]"><div className="text-2xl font-bold text-green-400">{sessionBookings.filter(b => b.status === 'Passed').length}</div><div className="text-[10px] uppercase text-green-300 font-bold">Passed</div></div>
                    </div>
                )}
            </div>
         </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-slate-100 dark:border-slate-700 overflow-hidden p-6 md:p-8">
          <div className="mb-8 max-w-2xl mx-auto">
            ...
            {availableSessions.length === 0 ? (
                <div className="bg-orange-50 text-orange-800 p-4 rounded-xl text-sm border border-orange-200 flex items-center justify-center gap-2"><AlertCircle size={16} /><span>{t.trainer.noSessions}</span></div>
            ) : (
                <div className="relative group">
                    <select value={selectedSessionId} onChange={handleSessionChange} className="w-full bg-slate-50 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-900 dark:text-white rounded-xl shadow-sm focus:border-yellow-500 focus:ring-yellow-500 p-4 pl-12 text-lg font-medium appearance-none cursor-pointer transition-colors hover:bg-slate-100 dark:hover:bg-slate-600">
                        <option value="">{t.trainer.chooseSession}</option>
                        {availableSessions.map(session => (
                            <option key={session.id} value={session.id}>{session.racType} • {session.date} • {session.location}</option>
                        ))}
                    </select>
            ...
            <div className="space-y-4">
                <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50 dark:bg-slate-700/50 p-3 rounded-xl border border-slate-200 dark:border-slate-600">
                    <div className="relative w-full md:w-64">
                        ...
                        <input type="text" placeholder="Find student..." className="w-full pl-9 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none text-black dark:text-white" value={studentSearch} onChange={(e) => setStudentSearch(e.target.value)} />
                    </div>
                    <button onClick={handleBulkAttendance} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 hover:text-blue-600 dark:hover:text-blue-400 transition-colors shadow-sm"><CheckSquare size={16} /> Mark All Present</button>
                </div>
                ...
                <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                    ...
                    <tr>
                        ...
                        {reqs.needsDl && <th className="px-4 py-4 text-center text-xs font-bold text-red-600 dark:text-red-400 uppercase tracking-wider w-28 bg-red-50 dark:bg-red-900/20">{t.trainer.table.dlCheck}</th>}
                        ...
                        {reqs.needsPractical && <th className="px-4 py-4 text-center text-xs font-bold text-black dark:text-slate-400 uppercase tracking-wider w-32">{t.trainer.table.practical}</th>}
                    ...
                    {filteredStudents.map((booking) => {
                        const theory = booking.theoryScore || 0;
                        const practicalLocked = reqs.needsPractical && theory < 70;
                        ...
                        <tr key={booking.id} className="hover:bg-blue-50/50 dark:hover:bg-slate-700/50 transition-colors group">
                            <td className="px-4 py-4"><div className="text-sm font-bold text-slate-900 dark:text-white">{booking.employee.name}</div><div className="text-xs text-slate-500 font-mono">{booking.employee.recordId}</div><div className="md:hidden text-[10px] text-slate-400 mt-1">{booking.employee.company} • {booking.employee.role}</div></td>
                            <td className="px-4 py-4 hidden md:table-cell text-xs text-slate-600 dark:text-slate-300">{booking.employee.company}</td>
                            <td className="px-4 py-4 hidden md:table-cell text-xs text-slate-600 dark:text-slate-300">{booking.employee.department}</td>
                            <td className="px-4 py-4 hidden md:table-cell text-xs text-slate-600 dark:text-slate-300">{booking.employee.role}</td>
                            ...
                            {reqs.needsDl && (
                                <td className="px-4 py-4 text-center bg-red-50/30 dark:bg-red-900/10">
                                    <input type="checkbox" className="h-5 w-5 text-red-600 focus:ring-red-500 border-red-300 rounded cursor-pointer" checked={booking.driverLicenseVerified || false} onChange={(e) => handleInputChange(booking.id, 'driverLicenseVerified', e.target.checked)} />
                                </td>
                            )}

                            <td className="px-4 py-4">
                            <input type="number" min="0" max="100" disabled={isDisqualified} className={`w-full text-center border-2 rounded-lg shadow-sm text-lg font-bold p-2 outline-none focus:ring-2 focus:ring-offset-1 transition-all ${isDisqualified ? 'bg-slate-100 dark:bg-slate-700 text-slate-400 border-slate-200 dark:border-slate-600' : (booking.theoryScore || 0) < 70 ? 'border-red-300 bg-red-50 dark:bg-red-900/20 text-red-700 focus:ring-red-500' : 'border-green-300 bg-green-50 dark:bg-green-900/20 text-green-700 focus:ring-green-500'}`} value={booking.theoryScore} onChange={(e) => handleInputChange(booking.id, 'theoryScore', parseInt(e.target.value) || 0)} />
                            </td>
                            {reqs.needsPractical && (
                                <td className="px-4 py-4 relative">
                                <input type="number" min="0" max="100" disabled={practicalLocked || isDisqualified} className={`w-full text-center border-2 rounded-lg shadow-sm text-lg font-bold p-2 outline-none focus:ring-2 focus:ring-offset-1 transition-all ${practicalLocked ? 'bg-slate-100 dark:bg-slate-700 text-slate-400 cursor-not-allowed border-slate-200' : (booking.practicalScore || 0) < 70 ? 'border-red-300 bg-red-50 dark:bg-red-900/20 text-red-700 focus:ring-red-500' : 'border-green-300 bg-green-50 dark:bg-green-900/20 text-green-700 focus:ring-green-500'}`} value={booking.practicalScore} onChange={(e) => handleInputChange(booking.id, 'practicalScore', parseInt(e.target.value) || 0)} />
                                {practicalLocked && <div className="absolute inset-0 flex items-center justify-center pointer-events-none opacity-40"><Lock size={14} className="text-slate-500" /></div>}
                                </td>
                            )}
                            <td className="px-4 py-4">
                            <span className={`px-3 py-1.5 inline-flex text-xs leading-5 font-black rounded-full shadow-sm border ${booking.status === BookingStatus.PASSED ? 'bg-green-100 text-green-800 border-green-200 dark:bg-green-900/30 dark:text-green-400 dark:border-green-800' : booking.status === BookingStatus.FAILED ? 'bg-red-100 text-red-800 border-red-200 dark:bg-red-900/30 dark:text-red-400 dark:border-red-800' : 'bg-slate-100 text-slate-500 border-slate-200 dark:bg-slate-700 dark:text-slate-400 dark:border-slate-600'}`}>
              ...
              <div className="p-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-end items-center gap-4 md:sticky md:bottom-0 z-10">
                 {successMsg && <span className="text-green-600 font-bold text-sm animate-pulse flex items-center gap-2"><CheckCircle2 size={16} /> {successMsg}</span>}
                 <button onClick={handleSave} disabled={!hasUnsavedChanges} className={`flex items-center space-x-2 px-8 py-3 rounded-xl font-bold shadow-lg transition-all transform hover:scale-105 ${hasUnsavedChanges ? 'bg-yellow-500 hover:bg-yellow-400 text-slate-900' : 'bg-slate-200 dark:bg-slate-700 text-slate-400 cursor-not-allowed shadow-none'}`}><Save size={20} /><span>{t.trainer.saveResults}</span></button>
              </div>
            </div>
          ) : (
             <div className="text-center py-20 text-slate-400">
                 <div className="bg-slate-50 dark:bg-slate-700 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-slate-600"><ClipboardList size={40} className="text-slate-300 dark:text-slate-500" /></div>
                 <p className="font-medium">{availableSessions.length === 0 ? "No pending sessions found assigned to you." : "Select a pending session to begin grading."}</p>
             </div>
          )}
      </div>
    </div>
