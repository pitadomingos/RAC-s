...
const App: React.FC = () => {
  ...
  const stampedSessions = useMemo(() => MOCK_SESSIONS.map(s => ({ ...s, siteId: 's1' })), []);
  ...
  }, [bookings, sessions]); // notifications dependency removed to avoid loops, intentionally checking inside setter

  // --- Handlers (Memoized for Performance) ---

  const handleUpdateRacDefinitions = useCallback((newDefs: RacDef[]) => {
      setRacDefinitions(newDefs);
  }, []);

  const clearNotifications = useCallback(() => setNotifications([]), []);

  const addNotification = useCallback((n: SystemNotification) => {
      setNotifications(prev => [n, ...prev]);
  }, []);

  // GENERIC HELPER: Sync Requirements from Bookings
  const syncRequirementsFromBookings = useCallback((bookingsToSync: Booking[]) => {
      setRequirements(prevReqs => {
          ...
          return changed ? newReqs : prevReqs;
      });
  }, [sessions]);
  ...
  }, [currentSiteId, syncRequirementsFromBookings]);

  const updateBookings = useCallback((updates: Booking[]) => {
      setBookings(prev => prev.map(b => {
          const update = updates.find(u => u.id === b.id);
          return update ? update : b;
      }));
      syncRequirementsFromBookings(updates);
  }, [syncRequirementsFromBookings]);

  const updateBookingStatus = useCallback((id: string, status: BookingStatus) => {
      setBookings(prev => prev.map(b => b.id === id ? { ...b, status } : b));
  }, []);

  const importBookings = useCallback((newBookings: Booking[], sideEffects?: { employee: Employee, aso: string, ops: Record<string, boolean> }[]) => {
  ...
  const updateRequirements = useCallback((req: EmployeeRequirement) => {
  ...
  const onUpdateEmployee = useCallback((id: string, updates: Partial<Employee>) => {
      setBookings(prev => prev.map(b => 
          b.employee.id === id ? { ...b, employee: { ...b.employee, ...updates } } : b
      ));
  }, []);

  const onDeleteEmployee = useCallback((id: string) => {
      setBookings(prev => prev.filter(b => b.employee.id !== id));
      setRequirements(prev => prev.filter(r => r.employeeId !== id));
  }, []);

  const onApproveAutoBooking = useCallback((bookingId: string) => {
  ...
  const onRejectAutoBooking = useCallback((bookingId: string) => {
